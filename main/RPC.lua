...
AddClientModRPCHandler("AdventureMode", "Adventure???", function(inst)
    local function yes()
    ...
    local buttons = {
        { text = STRINGS.UI.STARTADVENTURE.YES, cb = yes },
        { text = STRINGS.UI.STARTADVENTURE.NO, cb = no },
    }

    -- Huh?
    local bodytext = (require("stats").GetTestGroup() == 0 and STRINGS.UI.STARTADVENTURE.BODY) or STRINGS.UI.STARTADVENTURE.BODY_TEST

    local Screen = PopupDialogScreen(STRINGS.UI.STARTADVENTURE.TITLE, "Bruh", buttons, nil, "big")
